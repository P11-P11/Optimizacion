...
using MLDatasets, Images, Flux, Random, Statistics, Plots, InteractiveUtils,PlutoUI
...
# ╔═╡ 2bc4697c-49ec-4549-b004-cc9a84e90453
@bind l Slider(1:1:10)

# ╔═╡ 6dc9f1b8-41d9-47ac-9b64-59d4468b7c87
Gray.(ones(28,28)-X_prueba[:,:,findall(x-> x == 0,y_prueba)[l]]')

# ╔═╡ 57e0fded-2367-47bb-ad38-d309b2071efe
md""" 
!!! note "Agregamos visualizaciones de los primeros dígitos"
"""

# ╔═╡ 3c996a44-5591-4b86-a9d7-7c008110e1e1
Gray.(ones(28,28)-X_prueba[:,:,l]')
...
# ╔═╡ 93ec9034-543b-48e4-901d-8acbc8102a6b
function load_data(dataset, oneHot = false)
	
	Xₜᵣₐᵢₙ, yₜᵣₐᵢₙ = dataset(Float32, :train)[:]
	Xₜₑₛₜ, yₜₑₛₜ  = dataset(Float32, :test)[:]

	if oneHot
		yₜᵣₐᵢₙ = onehotbatch(yₜᵣₐᵢₙ, 0:9)
		yₜₑₛₜ  = onehotbatch(yₜₑₛₜ, 0:9)
	end

    return Xₜᵣₐᵢₙ, yₜᵣₐᵢₙ, Xₜₑₛₜ, yₜₑₛₜ
end

# ╔═╡ 86cbc906-ef00-4c5b-b05a-e8b1445f9cbb
X_train, y_train, X_test, y_test = load_data(MLDatasets.MNIST, true);
...
begin
predicha = onecold(model_prueba(Flux.unsqueeze(X_train[:,:,1],dims=3)))
real     = onecold(y_train[:,1])
println("La label predicha por nuestro modelo sin entrenar es ",predicha[1], ", la real es ", real)
end
...
begin
	plot([1:epochs],losses_train_descent,label="Descent", title="Loss en train set", xlabel="epoch", ylabel="loss")
...
md"""
!!! terminology "" 
	Veamos la perfomance de cada modelo
"""

# ╔═╡ 615e306b-bfc5-4943-9275-6e27d137daef
@bind metodo Slider(1:1:3)
...
plot!([1:epochs],losses_test[metodo],label=Metodos[metodo] * "test", mc=:red)
end
...
begin
X_train_, y_train_2, X_test_, y_test_2 = load_data(MLDatasets.MNIST, true)

X_train_2 = reshape(X_train_, 28, 28, 1,60_000)
X_test_2 = reshape(X_test_, 28, 28, 1, 10_000)

println("Dimensiones de X_train_2: ", size(X_train_2))
println("Dimensiones de X_test_2: ", size(X_test_2))
println("Dimensiones de y_train_2: ", size(y_train_2))
println("Dimensiones de y_test_2: ", size(y_test_2))
end
...
data2 = DataLoader((data=X_train_2, label=y_train_2), batchsize=100);
...
function model_conv()
    ...
    MaxPool((2,2)),                   # Segunda capa de MaxPool
    Flux.flatten,                     # Aplanar la salida para conectarla a la capa densa
    Dense(256 => 120, relu),          # Primera capa densa
    Dense(120 => 84, relu),           # Segunda capa densa
...
function train_model_conv!(model, loss, data, optimizador, epochs)
	...
	for epoch in 1:epochs
		Flux.train!(loss, parametros, data, optimizador);
		epoch_accuracy = accuracy(onecold(model(X_test_2)), onecold(y_test_2))
		epoch_loss_train = loss((X_train_2, y_train_2))
		epoch_loss_test = loss((X_test_2, y_test_2))
		@info "Epoch $epoch and loss $(epoch_loss_train)"
		@info "Epoch $epoch and accuracy $(epoch_accuracy)"
		accuracys[epoch] = epoch_accuracy
		losses_train[epoch] = epoch_loss_train
...
begin
	plot([1:epochs],losses_train_conv_descent,label="Descent train", mc=:blue, title="Loss en train vs test", xlabel="epoch", ylabel="loss", ma=0.75, ms=2)
	plot!([1:epochs],losses_test_conv_descent,label="Descent test", mc=:blue, shape=:hline)
	
	plot!([1:epochs],losses_train_conv_adam,label="Adam train", mc=:green, ma=0.75, ms=2)
	plot!([1:epochs],losses_test_conv_adam,label="Adam test", mc=:green, shape=:hline)
	
	plot!([1:epochs],losses_train_conv_momentum,label="Momentum train", mc=:pink, ma=0.75, ms=2)
	plot!([1:epochs],losses_test_conv_momentum,label="Momentum test", mc=:pink, shape=:hline)
end

# ╔═╡ a5926f68-74f4-4fd0-9f79-4daf7480d5d5
@bind metodo_conv Slider(1:1:3)
...
function preprocess_images(images::Array{Any,1})
    ...
    for img in images
		...
		preprocessed_img = reshape(img_array, 28, 28, 1, 1)
        push!(processed_images, preprocessed_img)
    end
...
for (i, pred) in enumerate(predictions)
    println("Image $(i-1): Predicted Digit = $(pred[1]-1)")
end

end

# ╔═╡ 6996ae05-4e35-4878-84f4-81d3bfdf5eb1
onecold(conv_descent(reshape(Float32.(images[5])', 28, 28, 1, 1)))
