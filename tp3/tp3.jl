...
begin
	...
	function gss(ϕ, a, b, eps, N)
	    ...
	    println("Failed requirements after $N steps")
	    return a, b
	end
...
# ╔═╡ 812f6a42-04d4-4d9c-8769-1ba39b1b1162
golden_section_search(t->t^2-t)
...
function L_aumentado(f, grad_f, g, grad_g, x0; tol=1e-5, max_iter=100, c=10.0)
    ...
    function L(x)
        ...
        gx = g(x)
        for j in 1:length(gx)
            P += (1 / (2c)) * (max(0, µ[j] + c * gx[j])^2 - µ[j]^2)
        end
        return f(x) + P
    end
    ...
    function ∇L(x)
        ...
        for j in 1:length(gx)
            if µ[j] + c * gx[j] > 0
                penalty_grad += (µ[j] + c * gx[j]) * grad_gx[j, :]
            end
        end
        return grad_f(x) + penalty_grad
    end
