...
begin function procesarDatos(path)
archivos = ["datos1.csv", "datos2.csv", "datos3.csv", "datos4.csv", "datos5.csv", 
            "datos6.csv", "datos7.csv", "datos8.csv", "datos9.csv", "datos10.csv"]

datos = []
directorio_base = path

for archivo in archivos
    ruta_archivo = joinpath(directorio_base, archivo)
    datos_aux = readdlm(ruta_archivo, ',')
    push!(datos, datos_aux)
end
...
# ╔═╡ 8a8c0346-b15e-4298-8504-8d7d6216a822
...
begin function def_problem(datos)
"
x^2+y^2 -2ax-2by -c = 0 ->
x^2+y^2 = 2ax + 2by + c
...
md"Usamos QR para resolver. Por motivos de optimización, no generamos explícitamente ninguna matriz y dejamos que Julia se encargue. Internamente, está haciendo algo así:
...
begin
function Σ_tilde_inversa(Σ,δ = 1e-8)
	n = length(Σ)
	res = zeros(n)
	σₘₐₓ = Σ[1]
	return diagm([σ > δ ? 1 / abs(σ) : δ/abs(σₘₐₓ) for σ in Σ])
end
function A⁺_factorizacion_positiva(A)
 u,s,v = svd(A)
 Σ⁺_ =  Σ_tilde_inversa(s)
 return v * Σ⁺_ * u' , s[1]/s[length(s)] # condicion
end
end
...
function ajuste_geom(θ₀,datos,factorizacion,max_iters = 10_000,error = 1e-9)
	θₖ   = θ₀
	θₖ₊₁ = θₖ 
	...
	if factorizacion == true
		A⁺ₖ , condicion = A⁺_factorizacion_positiva(Jₖ)	
	else
		A⁺ₖ , condicion = A⁺(Jₖ)
	end
	#d = d₀(θₖ,datos)
...
function ajuste_mejorado(datos)
	a,b,c = ajuste_alg(datos)
	r = sqrt(c + a^2 + b^2)
	return ajuste_geom([a,b,r], datos, true)
end
...
function plot_algebraico(datos, metodo)
    if metodo == "algebraico"
        ...
        radio = sqrt(c + a^2 + b^2)
		return plot_circulo(a,b,radio,datos,metodo)
	end
